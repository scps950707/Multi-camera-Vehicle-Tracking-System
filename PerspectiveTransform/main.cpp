...
int main( int argc, char *argv[] )
{
    ...
    cv::Point2f pts1[0] = {cv::Point2f( 0, 0 )};
    cv::Point2f pts1[1] = {cv::Point2f( 0, src_img.rows )};
    cv::Point2f pts1[2] = {cv::Point2f( src_img.cols, src_img.rows )};
    cv::Point2f pts1[3] = {cv::Point2f( src_img.cols, 0 )};

    cv::Point2f pts2[0] = {cv::Point2f( 0, 0 )};
    cv::Point2f pts2[1] = {cv::Point2f( 0 + nOffset, src_img.rows )};
    cv::Point2f pts2[2] = {cv::Point2f( src_img.cols - nOffset, src_img.rows )};
    cv::Point2f pts2[3] = {cv::Point2f( src_img.cols, 0 )};
